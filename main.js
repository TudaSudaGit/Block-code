...
function executeBlock(block) {
    try {
        ...
        const tryBlock = tryCatchStack.length > 0 ? tryCatchStack[tryCatchStack.length - 1].block : null;
        if (tryBlock) {
            tryBlock.classList.add('error-thrown');
            setTimeout(() => tryBlock.classList.remove('error-thrown'), 1000);
        }
